...
```php
patchServiceAsync($tenant_id, $service_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\Envelope
```
...
try {
    $result = $apiInstance->patchServiceAsync($tenant_id, $service_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
