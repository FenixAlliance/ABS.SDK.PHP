...
```php
patchServiceCaseTypeAsync($tenant_id, $service_case_type_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\Envelope
```
...
try {
    $result = $apiInstance->patchServiceCaseTypeAsync($tenant_id, $service_case_type_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
