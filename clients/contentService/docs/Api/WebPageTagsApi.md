...
```php
countWebPageTagsAsync($tenant_id, $api_version, $x_api_version): \OpenAPI\Client\Model\Int32Envelope
```
...
try {
    $result = $apiInstance->countWebPageTagsAsync($tenant_id, $api_version, $x_api_version);
    print_r($result);
} catch (Exception $e) {
