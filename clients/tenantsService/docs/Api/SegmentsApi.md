...
```php
patchTenantSegment($tenant_id, $tenant_segment_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\EmptyEnvelope
```
...
try {
    $result = $apiInstance->patchTenantSegment($tenant_id, $tenant_segment_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
