...
```php
patchTenantTeamContactEnrollment($tenant_id, $tenant_team_contact_enrollment_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\EmptyEnvelope
```
...
try {
    $result = $apiInstance->patchTenantTeamContactEnrollment($tenant_id, $tenant_team_contact_enrollment_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
