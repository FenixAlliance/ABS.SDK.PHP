...
```php
patchTenantSize($tenant_id, $tenant_size_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\EmptyEnvelope
```
...
try {
    $result = $apiInstance->patchTenantSize($tenant_id, $tenant_size_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
