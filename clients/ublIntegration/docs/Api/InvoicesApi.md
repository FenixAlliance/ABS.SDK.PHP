...
```php
apiV2UblServiceInvoicesInvoiceIdGet($tenant_id, $invoice_id, $profile, $api_version, $x_api_version)
```
...
try {
    $apiInstance->apiV2UblServiceInvoicesInvoiceIdGet($tenant_id, $invoice_id, $profile, $api_version, $x_api_version);
} catch (Exception $e) {
    echo 'Exception when calling InvoicesApi->apiV2UblServiceInvoicesInvoiceIdGet: ', $e->getMessage(), PHP_EOL;
}
```
