...
```php
getItemTagByIdAsync($item_tag_id, $tenant_id, $api_version, $x_api_version): \OpenAPI\Client\Model\ItemTagDtoEnvelope
```
...
try {
    $result = $apiInstance->getItemTagByIdAsync($item_tag_id, $tenant_id, $api_version, $x_api_version);
    print_r($result);
} catch (Exception $e) {
