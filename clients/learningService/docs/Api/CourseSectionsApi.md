...
```php
patchCourseSectionAsync($tenant_id, $section_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\EmptyEnvelope
```
...
try {
    $result = $apiInstance->patchCourseSectionAsync($tenant_id, $section_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
