...
```php
patchCourseAssignmentTypeAsync($tenant_id, $assignment_type_id, $api_version, $x_api_version, $operation): \OpenAPI\Client\Model\EmptyEnvelope
```
...
try {
    $result = $apiInstance->patchCourseAssignmentTypeAsync($tenant_id, $assignment_type_id, $api_version, $x_api_version, $operation);
    print_r($result);
} catch (Exception $e) {
