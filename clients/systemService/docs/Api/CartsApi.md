...
```php
getSystemCarts($api_version, $x_api_version): \OpenAPI\Client\Model\CartDtoListEnvelope
```
...
try {
    $result = $apiInstance->getSystemCarts($api_version, $x_api_version);
    print_r($result);
} catch (Exception $e) {
...
```php
getSystemCartsCount($api_version, $x_api_version): \OpenAPI\Client\Model\Int32Envelope
```
...
try {
    $result = $apiInstance->getSystemCartsCount($api_version, $x_api_version);
    print_r($result);
} catch (Exception $e) {
